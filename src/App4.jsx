...
const LOCATIONS = [
  { name: "Carrollton Gauge", lng: -90.1112, lat: 29.9499, type: "gauge" },
  { name: "Napoleon Ave Wharf", lng: -90.0856, lat: 29.9154, type: "terminal" },
  { name: "Algiers Point", lng: -90.0534, lat: 29.9271, type: "waypoint" },
  { name: "Globalplex / Associated Terminals", lng: -90.3674, lat: 29.9554, type: "terminal" },
  { name: "Southwest Pass", lng: -89.4073, lat: 28.9947, type: "waypoint" },
];

// Vessel starting position near Southwest Pass, moves toward NOLA
const VESSEL_ROUTE = [
  [-89.45, 28.98],
  [-89.8,  29.25],
  [-90.0,  29.65],
...
function RiverMap({ gaugeVal, statusColor }) {
  ...
  function initMap() {
    ...
    map.on("load", () => {
      ...
      LOCATIONS.forEach((loc) => {
        ...
        el.style.cssText = `
          width: ${loc.type === "gauge" ? 14 : 10}px;
          height: ${loc.type === "gauge" ? 14 : 10}px;
          border-radius: 50%;
          background: ${loc.type === "gauge" ? "#3bbfb2" : loc.type === "terminal" ? "#d97706" : "#4a7a75"};
          border: 2px solid ${loc.type === "gauge" ? "#3bbfb2" : loc.type === "terminal" ? "#d97706" : "#4a7a75"}44;
          box-shadow: 0 0 ${loc.type === "gauge" ? "12px" : "6px"} ${loc.type === "gauge" ? "#3bbfb244" : "#d9770644"};
          cursor: pointer;
        `;
        ...
        const popup = new window.mapboxgl.Popup({
          offset: 15,
          closeButton: false,
          className: "delta-popup",
        }).setHTML(`
          <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#3bbfb2;letter-spacing:0.08em;background:#0a0f0a;padding:6px 10px;border-radius:4px;border:1px solid #0f4547;">
            ${loc.name}
          </div>
        `);
        ...
        const marker = new window.mapboxgl.Marker({ element: el })
          ...
          .setPopup(popup)
          .addTo(map);
        markerRefs.current[loc.name] = { marker, el };
      });

      // Add vessel marker
      const vesselEl = document.createElement("div");
      vesselEl.innerHTML = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none">
        <polygon points="10,2 18,18 10,14 2,18" fill="#3bbfb2" stroke="#0a0f0a" stroke-width="1"/>
      </svg>`;
      vesselEl.style.cssText = "cursor:pointer;filter:drop-shadow(0 0 6px #3bbfb2);";
      const vesselPopup = new window.mapboxgl.Popup({
        offset: 15,
        closeButton: false,
      }).setHTML(`
        <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#3bbfb2;background:#0a0f0a;padding:6px 10px;border-radius:4px;border:1px solid #0f4547;">
          MV Delta Voyager<br/>LOA 185m · Draft 9.2m
        </div>
      `);
      vesselMarkerRef.current = new window.mapboxgl.Marker({ element: vesselEl })
        .setLngLat(VESSEL_ROUTE[0])
        .setPopup(vesselPopup)
        .addTo(map);

      // Animate vessel slowly along route
      let t = 0;
      function animateVessel() {
        t += 0.0003;
        if (t > 1) t = 0;
        const totalSegments = VESSEL_ROUTE.length - 1;
        const segmentT = t * totalSegments;
        const segIdx = Math.min(Math.floor(segmentT), totalSegments - 1);
        const localT = segmentT - segIdx;
        const from = VESSEL_ROUTE[segIdx];
        const to = VESSEL_ROUTE[segIdx + 1];
  ...
  // Update vessel color based on gauge level
  useEffect(() => {
    ...
    el.innerHTML = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none">
      <polygon points="10,2 18,18 10,14 2,18" fill="${color}" stroke="#0a0f0a" stroke-width="1"/>
    </svg>`;
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
          {[
            { color: C.teal, label: "Gauge Station" },
            { color: C.amber, label: "Terminal" },
            { color: C.muted, label: "Waypoint" },
            { color: statusColor, label: "MV Delta Voyager" },
          ].map(({ color, label }) => (
      ...
      {/* Popup styles */}
      <style>{`
        /* Gauge marker pulse - contained within element */
        @keyframes gaugePulse {
          0%   { box-shadow: 0 0 0 0 #3bbfb266; }
          70%  { box-shadow: 0 0 0 8px #3bbfb200; }
          100% { box-shadow: 0 0 0 0 #3bbfb200; }
        }
      ...
      >
        ...
        <div style={{ position: "relative", zIndex: 2, padding: "0 0 40px" }}>
          ...
          <div className="page-padding" style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {/* ── River Corridor Map ── */}
            <RiverMap gaugeVal={simGauge} statusColor={scenario.statusColor} />
