...
// Vessel route follows actual Mississippi River channel
// Corrected based on navigation map — river runs ~29.95 lat through NOLA crescent
const VESSEL_ROUTE = [
  ...
  [-90.5900, 30.0350],
  [-90.4700, 29.9900],
  [-90.3674, 29.9620], // Globalplex
  [-90.2900, 29.9600],
  [-90.2200, 29.9580],
  [-90.1700, 29.9560],
  [-90.1400, 29.9580], // Huey P Long Bridge
  [-90.1112, 29.9699], // Carrollton Gauge — adjusted north
  [-90.0880, 29.9760], // Audubon crescent peak
  [-90.0650, 29.9720],
  [-90.0450, 29.9600],
  [-90.0250, 29.9450],
  [-90.0050, 29.9280],
...
function RiverMap({ gaugeVal, statusColor, log }) {
  ...
  function initMap() {
    ...
    map.on("load", () => {
      // Tint water blue
      map.setPaintProperty("water", "fill-color", "#1a4a5c");

      // Fetch actual Mississippi River centerline from public API
      fetch("https://nominatim.openstreetmap.org/search?q=Mississippi+River&format=geojson&polygon_geojson=1&limit=1")
        .then(r => r.json())
        .then(data => {
          if (data?.features?.[0]) {
            map.addSource("corridor", { type: "geojson", data: data.features[0] });
            map.addLayer({
        ...
        .catch(() => {
          // Fallback: highlight existing water layer only
          if (map.getLayer("waterway-river")) {
            map.setPaintProperty("waterway-river", "line-color", "#3bbfb2");
            map.setPaintProperty("waterway-river", "line-width", 3);
          }
        });
